...
// Note that this parser must handle the edge case that an article is "uncategorized", like this one: https://skierscribbler.com/10076/uncategorized/scott-zevin/. It doesn't have a typical format.
class Parser {
    ...
    public func parseCategory(category: SwiftSoup.Document) -> [Article] {
        do {
            return try _parseCategory(category)
        } catch {
    ...
    private func _parseCategory(_ doc: Document) throws -> [Article] {
        //TODO: Change to `Elements` to continue with the native SwiftSoup Types?
        var articleHTMLs: [Element] = []
        try doc.getElementsByTag("div").forEach({ el in
            if el.hasClass("catlist-panel-inner") {
                articleHTMLs.append(el)
            }
        })
        
        return try articleHTMLs.map { el in
            try _parseArticleSubHTML(el)
        }
    }
    
    func _parseArticleSubHTML(_ parentHTML: Element) throws -> Article {
        var id: Int
